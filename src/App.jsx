...
function OrderSummary({ onOpenCatalog }) {
  return <section className="order-summary-card home-order-cta"><h3>Armar comanda para mañana</h3><button type="button" className="dark-button" onClick={onOpenCatalog}>Ir al catálogo <ArrowRight size={17}/></button></section>
}

function BakeryHome({ quantities, orders, setView, onRepeat, onLogout }) {
  const lastOrders = orders.slice(0, 2)
  const cartQuantity = Object.values(quantities).reduce((sum, value) => sum + value, 0)
  return <><Topbar title="Buenos días" subtitle="Tu operación conectada a Seven Pan" cartQuantity={cartQuantity} onOpenCart={() => setView('catalog')} /><main className="page-content home-content"><section className="home-order-actions" aria-label="Acciones de comanda"><OrderSummary onOpenCatalog={() => setView('catalog')} /><button type="button" className="repeat-order-card" onClick={() => onRepeat(orders[0])} disabled={!orders[0]}><Copy size={18}/><span>Repetir comanda anterior</span></button></section><section className="history-section home-history-section"><div className="section-heading"><div><span className="section-label">ÚLTIMAS COMANDAS</span><h3>Historial de pedidos</h3></div><button className="link-button" onClick={() => setView('orders')}>Ver todos <ArrowRight size={15}/></button></div><div className="history-table">{lastOrders.map((order) => <div className="history-row" key={order.id}><div className="date-square"><span>{formatDate(order.delivery_date).split(' ')[0]}</span><b>{new Date(`${order.delivery_date}T12:00:00`).getDate()}</b></div><div className="history-details"><strong>Pedido {orderNumber(order)}</strong><span>{orderItemsText(order)}</span></div><strong className="history-price">{formatMoney(order.total)}</strong><Status status={order.status}/><button className="round-action" onClick={() => onRepeat(order)} aria-label={`Repetir pedido ${orderNumber(order)}`}><Copy size={16}/></button></div>)}{!lastOrders.length && <div className="empty-order"><p>Todavía no tenés comandas registradas.</p></div>}</div></section><button type="button" className="customer-home-logout" onClick={onLogout}><LogOut size={18}/> Cerrar sesión</button></main></>
}
  ...
  return <>
    ...
    <main className="page-content catalog-layout">
      <div className="catalog-main">
        ...
        <div className="product-list">{filtered.map((product) => <article className="product-card" key={product.id}><div className={`product-art ${product.tone}`}><div className={`product-shape ${product.id}`}><i/><i/><i/></div></div><div className="product-info"><h3>{product.name}</h3><p>{product.detail}</p><strong>{formatMoney(product.price)} <small>/ {product.unit}</small></strong></div><div className="quantity-control"><button aria-label={`Quitar una unidad de ${product.name}`} onClick={() => updateQuantity(product.id, -1)} disabled={!quantities[product.id]}>−</button><b>{quantities[product.id] || 0}</b><button aria-label={`Agregar una unidad de ${product.name}`} onClick={() => updateQuantity(product.id, 1)}>+</button><span>{product.unit}</span></div></article>)}</div>
      </div>
      <aside className="order-panel">
        ...
        <div className="order-total"><span>Total estimado</span><strong>{formatMoney(total)}</strong><small>Se paga al momento de la entrega.</small></div>
        <button disabled={!units || submitting} className="primary-button confirm-order" onClick={() => onSubmit({ deliveryDate: editingOrder?.delivery_date || scheduledDeliveryDate(), notes: editingOrder?.notes || '' })}>{submitting ? 'Guardando…' : editingOrder ? 'Guardar cambios' : 'Confirmar comanda'}</button>
...
function BakeryOrderCard({ order, onRepeat, onCancel, cancelling }) {
  const [open, setOpen] = useState(false)
  return <article className={`order-card bakery-order-card ${open ? 'open' : ''}`}><div className="bakery-order-summary"><div className="order-card-main"><div className="order-number"><Package size={20}/></div><div><div className="order-title"><h3>Pedido {orderNumber(order)}</h3><Status status={order.status}/></div><p>{orderItemsText(order)}</p></div></div><div className="bakery-order-side"><time className="bakery-order-date" dateTime={order.delivery_date}>{formatDate(order.delivery_date)}</time><button className="secondary-button compact bakery-repeat-button" onClick={() => onRepeat(order)}><Copy size={16}/> Repetir</button></div><div className="bakery-order-actions">{order.status === 'pending' && <button className="icon-text-button danger" disabled={cancelling} onClick={() => onCancel(order)}><X size={15}/> Cancelar</button>}<button type="button" className="bakery-order-detail-toggle" aria-expanded={open} aria-controls={`bakery-order-${order.id}`} onClick={() => setOpen((value) => !value)}><span>{open ? 'Ocultar' : 'Ver detalle'}</span><ChevronDown size={17}/></button></div><div className="order-card-total"><small>TOTAL</small><strong>{formatMoney(order.total)}</strong></div></div>{open && <div className="bakery-order-detail" id={`bakery-order-${order.id}`}><OrderDetail order={order} showHeader={false}/></div>}</article>
}

function Orders({ orders, onRepeat, onCancel, cancellingId }) {
  return <><Topbar title="Mis pedidos" subtitle="Consultá o repetí tus comandas"/><main className="page-content orders-page"><section className="orders-heading"><div><span className="eyebrow warm">HISTORIAL</span><h2>Tus últimas comandas</h2></div><button className="secondary-button" onClick={() => onRepeat(orders[0])} disabled={!orders[0]}><Copy size={17}/> Repetir último pedido</button></section><div className="order-list">{orders.map((order) => <BakeryOrderCard order={order} onRepeat={onRepeat} onCancel={onCancel} cancelling={cancellingId === order.id} key={order.id}/>)}{!orders.length && <p className="empty-copy">Todavía no hay pedidos para mostrar.</p>}</div></main></>
}
...
function AdminWorkspace({ orders, customers, products, onStatusChange, onProductSave, onProductCreate, onCustomerCreate, onCustomerAccessUpdate, onCustomerAccessCodeCreate, onCustomerStatusChange, onCustomerDelete, onPasswordChange, savingOrderId, savingProductId, creatingProduct, creatingCustomer, savingCustomerId, onLogout }) {
  ...
  const overview = <div className="dashboard-overview" key={period}>
    ...
    <button type="button" className="admin-mobile-logout" onClick={onLogout}><LogOut size={18}/> Cerrar sesión</button>
  </div>
...
export default function App() {
  ...
  const pages = {
    home: <BakeryHome quantities={quantities} products={workspace.products} orders={workspace.orders} setView={navigateView} onRepeat={repeat} onLogout={handleLogout}/>,
    ...
    orders: <Orders orders={workspace.orders} onRepeat={repeat} onCancel={cancelOrder} cancellingId={savingOrderId}/>,
